...
$(document).ready(function () {
  ...
  $(
    "<div class = 'totalParks '>Total National Parks in State: </div>"
  ).appendTo(".container");
  ...
  $.ajax({
    ...
    arrayResponse.forEach(function (site, index) {
      ...
      $.ajax({
        ...
        let F = (weatherTemp - 273.15) * 1.8 + 32;
        F = F.toFixed(0);
      ...
      let campsiteTotals = $(
        "<p>" + "Total number of campsites: " + site.campsites + "</p>"
      );
      let fees = site.fees;
      let dollarFees = parseFloat(fees).toFixed(2);
      let finalFees = $(
        "<p>" + "Extimated Campsite Fee: $" + dollarFees + "</p>"
      );
      let p_id = site.p_id;
      console.log(p_id);
    ...
    totalparks.textContent =
      "Total Nat. Park Campgrounds in State: " + response.total;
  });
});
